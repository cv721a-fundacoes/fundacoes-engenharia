...
Será fornecido um dataset-base oficial da disciplina, bem como o workflow do software Orange a ser utilizado.

O uso da regressão linear é obrigatório nesta etapa.

O objetivo é introduzir conceitos fundamentais de aprendizagem supervisionada, avaliação de erro e interpretação física dos resultados.

Alterações não autorizadas no workflow ou utilização de modelos adicionais não geram pontuação extra.

A aplicação de Machine Learning não substitui o raciocínio de engenharia, o dimensionamento normativo ou o julgamento técnico do projetista.
...
Desenvolver um projeto completo de fundações profundas, integrando dimensionamento clássico, análises no software RSPile, Machine Learning em nível intermediário e uso integrado de BIM com apoio do pyRevit.

\subsection*{Descrição do Problema}

Cada grupo receberá dados de sondagem SPT, cargas estruturais e um tipo específico de fundação profunda (tubulão) definido pelo professor, mantendo a prática tradicional da disciplina.
...
\subsubsection*{BIM e Integração de Dados}

Modelagem dos tubulões no Revit, com uso de pyRevit para leitura de parâmetros, verificação de coerência geométrica e extração de informações do modelo.

A integração entre dados, hipóteses adotadas e representação BIM caracteriza um sistema digital de apoio ao projeto, sem substituir o dimensionamento clássico.
